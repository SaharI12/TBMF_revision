...
class TryDataset_v0(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        labels_idx[5, :, :] = labels_idx[5, :, :] / 1000 # T2
...
def create_dataloader_per_volunteer(vol_path):
    ...
    data_paths = sorted(glob.glob(os.path.join(vol_path, "*/axial/*/dataset/*.mat")),
                        key=os.path.getmtime)
    param_paths = sorted(glob.glob(os.path.join(vol_path, "*/axial/*/params/*.mat")),
                         key=os.path.getmtime)
    label_paths = sorted(glob.glob(os.path.join(vol_path, "*/axial/*/labels/*.mat")),
                         key=os.path.getmtime)
    print(len(data_paths), len(param_paths), len(label_paths))
